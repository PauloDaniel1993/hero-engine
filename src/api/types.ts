...
export type EngineEvent =
  ...
  | "crit-dealt"        // this actor rolled a critical hit
  | "crit-received"     // this actor was critically hit
  | "reduced-to-zero"   // this actor reduced an enemy to 0 HP
  | "damage-taken"      // this actor took damage
  | "ally-downed"       // a friendly actor dropped to 0 HP nearby
  | "rest-short"
  | "rest-long"
  | "turn-start"
  | "turn-end"
...
export interface PromptDef {
  ...
  bodyKey?: string;
  save?: SavePromptDef;
  choices?: { id: string; labelKey: string; apply?: StateOp[]; chatKey?: string }[];
...
export interface ActionDef {
  ...
  descriptionKey?: string;
  gmOnly?: boolean;
  costs?: { resource: string; amount: NumberOrFormula }[];
  apply?: StateOp[];
...
export interface StanceGroupDef {
  ...
  switchCost?: { resource: string; amount: NumberOrFormula };
  allowNone?: boolean;
  stances: StanceDef[];
}
...
export interface TransformDef {
  ...
  overlay?: { effects: Record<string, unknown>[]; grantItems?: Record<string, unknown>[] };
  swap?: { formActorName: string; hpCarry: "keep-percent" | "form-max" };
...
export interface ConsequenceTableDef {
  ...
  /** e.g. "1d6" */
  die: string;
  entries: { min: number; max: number; textKey: string; apply?: StateOp[] }[];
}
...
export interface AdjudicationDef {
  ...
  kind: "confirm" | "choice";
  /** For `choice`. */
  choices?: { id: string; labelKey: string; apply?: StateOp[] }[];
  /** For `confirm`. */
  onConfirm?: StateOp[];
  onDeny?: StateOp[];
  runHook?: boolean;
...
export interface StateAccessor {
  ...
  transform(): { id: string; roundsLeft: number } | null;
}
...
export interface MechanicContext {
  ...
  rollTable(tableId: string): Promise<{ roll: number; textKey: string }>;
  ...
  postCard(opts: {
    ...
    buttons: { labelKey: string; actionId: string }[];
  }): Promise<void>;
...
export interface HeroEngineAPI {
  ...
  attach(actor: ActorDoc, pluginId: string, options?: { item?: ItemDoc }): Promise<void>;
